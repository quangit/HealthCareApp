...
namespace HealthCare.Droid.Renderers
{
    public class WebViewCustomRenderer : WebViewRenderer
    {
        ...
        private new WebViewCustom Element { get { return (WebViewCustom)base.Element; } }
        ...
        {
            private readonly WebViewCustomRenderer _renderer;
            private readonly Android.Webkit.WebViewClient _originalClient;

            public AuthenticatingWebViewClient(WebViewCustomRenderer renderer, Android.Webkit.WebViewClient originalClient)
            {
                _renderer = renderer;
            ...
            public override void OnPageStarted(Android.Webkit.WebView view, string url, Android.Graphics.Bitmap favicon)
            {
                if (_originalClient != null)
                {
                    if (AppConstant.IsChangePin)
                        PasswordViewModel.Instance.ChangePinNavigating(_renderer._webView, url);
                    else
                    {
                        CreditCardViewModel.Instance.AddNewCardNavigating(_renderer._webView, url);
                    }
