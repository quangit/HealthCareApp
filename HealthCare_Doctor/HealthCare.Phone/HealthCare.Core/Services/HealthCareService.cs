...
namespace HealthCare.Core.Services
{
	public class HealthCareService
	{
		...
		public async Task<LoginResult> LoginAsync(string id, string password)
		{
			...
			var ret = await PostApiCall<LoginResult>("doctor/login", new { id = id.ToLower(), password });
			if (ret != null)
			{
		...
		{
			return
				GetApiCall<List<Hospital>>(string.Format("doctor/refreshData"), "hospitals");
		}
		public Task<List<Schedule>> GetShedules(string hospitalId = "")
		{
			...
			return
				GetApiCall<List<Schedule>>(
					string.Format(
						"doctor/{0}/schedules?hospitalId={1}",
						Data.User.UserId,
						hospitalId), null, "schedules");
		}
		...
		public Task<ConsultRequestData> GetRequests(int start = 0)
		{
			...
			var resp = GetApiCall<ConsultRequestData>(string.Format("doctor/supportRequests?start={0}&length={1}" + TabletStrParam(), start, Data.LoadLength));
			return resp;
		}
		private string TabletStrParam()
		{
			return (Data.Platform == Data.Os.iOS || Data.Platform == Data.Os.Android) ? "" : "&device=tablet";
		}
        public Task<ConsultRequsetDetail> GetRequestDetail(string id)
        {	
            return GetApiCall<ConsultRequsetDetail>("doctor/supportRequest/" + id + "/responses" + TabletStrParam());
        }

        public Task<Doctor> GetProfile()
        {
            return GetApiCall<Doctor>(string.Format("user/{0}/get", Data.User.UserId)
                , null, "user");
        }
            ...
            bool noProgress = false)
        {
            ...
            try
            {
                ...
                api += ((api.Contains("?")) ? "&" : "?"  ) + "ignore=" + DateTime.Now.Ticks;
