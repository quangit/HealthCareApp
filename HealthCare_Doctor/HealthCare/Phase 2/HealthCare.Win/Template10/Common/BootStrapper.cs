...
namespace Template10.Common
{
    public abstract class BootStrapper : Application, INotifyPropertyChanged
    {
        ...
        protected sealed override async void OnActivated(IActivatedEventArgs e) { DebugWrite(); await InternalActivatedAsync(e); }
        ...
        private void RaiseBackRequested(ref bool handled)
        {
            ...
            if (ChbaseView.Current != null && ChbaseView.Current.RootWebView.CanGoBack)
            {
                ChbaseView.Current.RootWebView.GoBack();
                return;
            }
        ...
        private async Task InitializeFrameAsync(IActivatedEventArgs e)
        {
            ...
            if (Window.Current.Content == null || Window.Current.Content == splash)
            {
                ...
                frame = (NavigationServiceFactory(BackButton.Attach, ExistingContent.Include, frame)).FrameFacade.Frame;
        ...
        public virtual Frame CreateRootFrame(IActivatedEventArgs e)
        {
